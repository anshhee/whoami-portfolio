...
const DEFAULT_CONFIG: Required<Omit<GradualBlurConfig, 'preset' | 'hoverIntensity' | 'onAnimationComplete' | 'width' | 'mobileHeight' | 'tabletHeight' | 'desktopHeight' | 'mobileWidth' | 'tabletWidth' | 'desktopWidth'>> = {
    position: 'bottom',
    strength: 2,
    height: '6rem',
    divCount: 5,
...
const useResponsiveDimension = (responsive: boolean | undefined, config: GradualBlurConfig, key: 'height' | 'width') => {
...
function GradualBlur(props: GradualBlurConfig) {
    ...
    useEffect(() => {
        if (isVisible && config.animated === 'scroll' && config.onAnimationComplete) {
            const ms = parseFloat(config.duration ?? '0.3') * 1000;
